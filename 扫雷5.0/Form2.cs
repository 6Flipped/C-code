...
namespace 扫雷4._0
{
    public partial class Form2 : Form
    {
        ...
        private int mineWidth = 28;//单个小按钮的边长
        private int xnum = 20;//16列
        private int ynum = 13;//16行
        private int minenum = 50;//雷的个数。
        private int[,] flags;
        private int[,] MINES;
            ...
            for (int i = xmin; i <= xmax; i++)
            {
                for (int j = ymin; j <= ymax; j++)
                {
                    ...
                    else
                    {
                        ...
                        if (count == 4)
                            minebtns[i, j].BackgroundImage = Image.FromFile("4.gif");
                        if (count == 5)
                            minebtns[i, j].BackgroundImage = Image.FromFile("5.gif");
                        if (count == 6)
                            minebtns[i, j].BackgroundImage = Image.FromFile("6.gif");
                        if (count == 7)
                            minebtns[i, j].BackgroundImage = Image.FromFile("7.gif");
                        if (count == 8)
                            minebtns[i, j].BackgroundImage = Image.FromFile("8.gif");
